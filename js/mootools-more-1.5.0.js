/*
 ...
 web build:
 - http://mootools.net/more/ddb8907168cf1c7b22f906b12b681bf6

 packager build:
 - packager build More/More More/Events.Pseudos More/Class.Refactor More/Class.Binds More/Class.Occlude More/Chain.Wait More/Array.Extras More/Date More/Date.Extras More/Number.Format More/String.Extras More/String.QueryString More/URI More/URI.Relative More/Hash More/Hash.Extras More/Element.Forms More/Elements.From More/Element.Event.Pseudos More/Element.Event.Pseudos.Keys More/Element.Measure More/Element.Pin More/Element.Position More/Element.Shortcuts More/Form.Request More/Form.Request.Append More/Form.Validator More/Form.Validator.Inline More/Form.Validator.Extras More/OverText More/Fx.Elements More/Fx.Accordion More/Fx.Move More/Fx.Scroll More/Fx.Slide More/Fx.SmoothScroll More/Fx.Sort More/Drag More/Drag.Move More/Slider More/Sortables More/Request.JSONP More/Request.Queue More/Request.Periodical More/Assets More/Color More/Group More/Table More/HtmlTable More/HtmlTable.Zebra More/HtmlTable.Sort More/HtmlTable.Select More/Keyboard More/Keyboard.Extras More/Mask More/Scroller More/Tips More/Spinner More/Locale More/Locale.en-US.Date More/Locale.en-US.Form.Validator More/Locale.en-US.Number More/Locale.de-CH.Date More/Locale.de-CH.Form.Validator More/Locale.de-DE.Date More/Locale.de-DE.Form.Validator More/Locale.de-DE.Number More/Locale.EU.Number
...
(function () {
    ...
    var e = this.Locale = {define: function (f, j, h, i) {
        var g;
        if (instanceOf(f, e.Set)) {
            g = f.name;
            if (g) {
    ...
    e.Set = new Class({sets: {}, inherits: {locales: [], sets: {}}, initialize: function (f) {
        this.name = f || "";
    ...
    Element.implement({pin: function (h, f) {
        ...
        if (h !== false) {
            ...
            if (!this.retrieve("pin:_pinned")) {
                var g = {top: j.y - k.y, left: j.x - k.x, margin: "0px", padding: "0px"};
                if (a && !f) {
...
(function () {
    var a = false;
    this.IframeShim = new Class({Implements: [Options, Events, Class.Occlude], options: {className: "iframeShim", src: 'javascript:false;document.write("");', display: false, zIndex: null, margin: 0, offset: {x: 0, y: 0}, browsers: a}, property: "IframeShim", initialize: function (c, b) {
        ...
        if (this.options.browsers) {
            ...
            if (!d) {
                d = 1;
                var c = this.element.getStyle("position");
                if (c == "static" || !c) {
                    this.element.setStyle("position", "relative");
                }
                this.element.setStyle("zIndex", d);
            }
            d = ((this.options.zIndex != null || this.options.zIndex === 0) && d > this.options.zIndex) ? this.options.zIndex : d - 1;
            if (d < 0) {
                d = 1;
            }
            this.shim = new Element("iframe", {src: this.options.src, scrolling: "no", frameborder: 0, styles: {zIndex: d, position: "absolute", border: "none", filter: "progid:DXImageTransform.Microsoft.Alpha(style=0,opacity=0)"}, "class": this.options.className}).store("IframeShim", this);
            var b = (function () {
...
(function () {
    ...
    }, opacity: 1, hideInputs: !("opacity" in document.documentElement) ? "select, input, textarea, object, embed" : null}, dissolve: function () {
...
Form.Validator = new Class({Implements: [Options, Events], options: {fieldSelectors: "input, select, textarea", ignoreHidden: true, ignoreDisabled: true, useTitles: false, evaluateOnSubmit: true, evaluateFieldsOnBlur: true, evaluateFieldsOnChange: true, serial: true, stopOnFailure: true, warningPrefix: function () {
    ...
    this._bound = {onSubmit: this.onSubmit.bind(this), blurOrChange: function (c, d) {
...
Form.Validator.addAllThese([
    ...
    ["validate-date", {errorMsg: function (a, b) {
        ...
        var a = Locale.get("Date"), b = new RegExp([a.days, a.days_abbr, a.months, a.months_abbr, a.AM, a.PM].flatten().join("|"), "i"), i = e.get("value"), f = i.match(/[a-z]+/gi);
...
Fx.Accordion = new Class({Extends: Fx.Elements, options: {fixedHeight: false, fixedWidth: false, display: 0, show: false, height: true, width: false, opacity: true, alwaysHide: false, trigger: "click", initialDisplayFx: true, resetHeight: true}, initialize: function () {
    ...
    if (b == this.current && !a.alwaysHide) {
        return this;
    }
    if (a.resetHeight) {
        var e = g[this.current];
    ...
    if ((this.timer && a.link == "chain") || (b === this.current && !a.alwaysHide)) {
        return this;
    }
...
Drag.Move = new Class({Extends: Drag, options: {droppables: [], container: false, precalculate: false, includeMargins: true, checkDroppables: true}, initialize: function (b, a) {
    ...
    if (j.getStyle("position") == "relative") {
        ...
        if (e != d) {
            ...
            l += d == document.body ? 0 : k.top + m.top;
            if (!m.top && l < 0) {
                l = 0;
            }
        }
...
var Slider = new Class({Implements: [Events, Options], Binds: ["clickedElement", "draggedKnob", "scrolledElement"], options: {onTick: function (a) {
    ...
    this.previousChange = this.previousEnd = this.step = e.initialStep ? e.initialStep : e.range ? e.range[0] : 0;
    ...
    this.setRange(e.range, null, true);
    ...
    var a = this.stepSize = Math.abs(this.range) / this.steps;
    this.stepWidth = this.stepSize * this.full / Math.abs(this.range);
    this.setModulus();
    if (b) {
        this.set(Array.pick([d, this.step]).limit(this.min, this.max), c);
    }
    return this;
}, setModulus: function () {
    var a = ((this.stepSize + "").split(".")[1] || []).length, b = 1 + "";
    while (a--) {
        b += "0";
    }
...
var Sortables = new Class({Implements: [Events, Options], options: {opacity: 1, clone: false, revert: false, handle: false, dragOptions: {}, unDraggableTags: ["button", "input", "a", "textarea", "select", "option"]}, initialize: function (a, b) {
    this.setOptions(b);
    this.elements = [];
    this.lists = [];
    this.idle = true;
    ...
    var b = c.getPosition(d);
    var a = {w: window.getScroll(), offsetParent: d.getScroll()};
    b.x += a.offsetParent.x;
    b.y += a.offsetParent.y;
    if (d.getStyle("position") == "fixed") {
        b.x -= a.w.x;
        b.y -= a.w.y;
    }
    return b;
    ...
    if (!this.idle || b.rightClick || (!this.options.handle && this.options.unDraggableTags.contains(b.target.get("tag")))) {
        return;
    }
    this.idle = false;
    this.element = a;
    ...
    this.drag = new Drag.Move(this.clone, Object.merge({droppables: this.getDroppables()}, this.options.dragOptions)).addEvents({onSnap: function () {
        b.stop();
    ...
    if (this.effect) {
        ...
        var b = function () {
            this.removeEvent("cancel", b);
            e.destroy();
            a.reset();
        };
        this.effect.element = e;
        this.effect.start({top: d.top, left: d.left, width: c.width, height: c.height, opacity: 0.25}).addEvent("cancel", b).chain(b);
    } else {
        this.clone.destroy();
        a.reset();
    }
...
var HtmlTable = new Class({Implements: [Options, Events, Class.Occlude], options: {properties: {cellpadding: 0, cellspacing: 0, border: 0}, rows: [], headers: [], footers: []}, property: "HtmlTable", initialize: function () {
    ...
    e.each(function (i, f) {
        ...
        if (i && Object.prototype.hasOwnProperty.call(i, "properties")) {
            j.set(i.properties);
        }
...
(function () {
    ...
    HtmlTable = Class.refactor(HtmlTable, {options: {sortIndex: 0, sortReverse: false, parsers: [], defaultParser: "string", classSortable: "table-sortable", classHeadSort: "table-th-sort", classHeadSortRev: "table-th-sort-rev", classNoSort: "table-th-nosort", classGroupHead: "table-tr-group-head", classGroup: "table-tr-group", classCellSort: "table-td-sort", classSortSpan: "table-th-sort-span", sortable: false, thSelector: "th"}, initialize: function () {
        ...
        var h = new Element("span", {"class": this.options.classSortSpan}).inject(e, "top");
        this.sortSpans.push(h);
        var i = this.options.parsers[d], g = this.body.rows, f;
        switch (typeOf(i)) {
            case"function":
                i = {convert: i};
                f = true;
        ...
        if (!f) {
            HtmlTable.ParserPriority.some(function (l) {
                ...
                for (var o = 0, m = g.length; o < m; o++) {
                    var k = document.id(g[o].cells[d]), q = k ? k.get("html").clean() : "";
                    if (q && n.test(q)) {
                        i = p;
                        return true;
                    }
        ...
        while (g) {
            var j = d[--g], e = j.position, k = f.rows[e];
            if (k.disabled) {
                continue;
            }
            if (!c) {
                i = this.setGroupSort(i, k, j);
                this.setRowStyle(k, g);
            }
            f.appendChild(k);
            for (h = 0;
                 h < g; h++) {
        ...
        if (!g) {
            this.clearSort();
            this.setSortedState(e, d);
            this.setHeadSort(true);
        }
        var h = this.getParser();
        if (!h) {
            return;
        }
        var c;
        ...
        var f = this.parseData(h).sort(function (j, i) {
            if (j.value === i.value) {
                return 0;
            }
            return j.value > i.value ? 1 : -1;
        });
        if (this.sorted.reverse == (h == HtmlTable.Parsers["input-checked"])) {
            f.reverse(true);
        }
        this.setRowSort(f, g);
        if (c) {
